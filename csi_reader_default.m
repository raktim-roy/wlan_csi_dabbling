...
BW = 80;                % bandwidth
FILE = './txpower_0db_1.pcap';% capture file
NPKTS_MAX = 14000;       % max number of UDPs to process
...
while (k <= n)
    ...
    Hout = reshape(Hout,2,[]).';
    cmplx = double(Hout(1:NFFT,2))+1j*double(Hout(1:NFFT,1));
    csi_buff(k,:) = cmplx.';
    k = k + 1;
end

csi_mag =abs(csi_buff);
csi_single_carrier = csi_mag(:,180);
figure()
plot(csi_single_carrier);
